...
def demo_fn(args):
    ...
    if os.path.exists(os.path.join(target_scene_dir, "matches.pt")):
        ...
        match_outputs = opt_utils.vggt_extract_matches(model, extrinsic, intrinsic,depth_conf, track_feats, 
                                        base_image_path_list, args.max_query_pts, device=device, dtype=dtype)
