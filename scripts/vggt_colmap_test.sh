...
scene_dir="data/TNT_GOF/TrainingSet/Courthouse"
post_fix="_vggt_opt_speed_test"

for img_num in {100..1000..100}; do
    while [ -d "$scene_dir" ]; do
        ...
        if [[ -n $gpu_id ]]; then
            ...
            CUDA_VISIBLE_DEVICES=$gpu_id python demo_colmap_ratio_opt.py \
                --scene_dir $scene_dir \
                --post_fix ${post_fix}_$img_num \
                --total_frame_num $img_num \
                --shared_camera \
                --use_opt & \
